...
RSpec.describe Order do
  ...
  describe 'instance methods' do
    before :each do
      ...
      @ogre = @monster_shop.items.create!(name: 'Ogre', description: "I'm an Ogre!", price: 20.25, image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTaLM_vbg2Rh-mZ-B4t-RSU9AmSfEEq_SN9xPP_qrA2I6Ftq_D9Qw', active: true, inventory: 3 )
    ...
    it '.merchant_subtotal()' do
      expect(@order_2.merchant_subtotal(@monster_shop.id)).to eq(40.5)
      expect(@order_2.merchant_subtotal(@pet_shop.id)).to eq(100)
    end

    it '.merchant_quantity()' do
      expect(@order_2.merchant_quantity(@monster_shop.id)).to eq(2)
      expect(@order_2.merchant_quantity(@pet_shop.id)).to eq(2)
    end
